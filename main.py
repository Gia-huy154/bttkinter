...
def show_today_birthdays():
    ...
    if birthdays:
        message = "\n".join(
            [f"Mã: {e['Mã']}, Tên: {e['Tên']}" for e in birthdays]
        )
        messagebox.showinfo("Sinh nhật hôm nay", message)
    else:
        messagebox.showinfo(
            "Sinh nhật hôm nay", "Không có nhân viên nào sinh nhật hôm nay."
        )
...
def export_to_excel():
    try:
        ...
        df["Tuổi"] = df["Ngày sinh"].apply(
            lambda x: (datetime.now() - datetime.strptime(x, "%d/%m/%Y")).days
            // 365
        )
        ...
        messagebox.showinfo(
            "Thành công", f"Danh sách nhân viên đã xuất ra file {output_file}"
        )
...
checkbox1 = tk.Checkbutton(
    root, text="Là khách hàng", variable=var1, font=("Arial", 8)
)
checkbox1.place(x=130, y=6)
var2 = tk.BooleanVar()
checkbox2 = tk.Checkbutton(
    root, text="Là nhà cung cấp", variable=var2, font=("Arial", 8)
)
...
gender_male = ttk.Radiobutton(
    frame, text="Nam", variable=gender_var, value="Nam"
)
gender_female = ttk.Radiobutton(
    frame, text="Nữ", variable=gender_var, value="Nữ"
)
widgets = [
    ("Mã", entry_id),
    ("Tên", entry_name),
...
btn_birthdays = ttk.Button(
    btn_frame, text="Sinh nhật hôm nay", command=show_today_birthdays
)
btn_birthdays.grid(row=0, column=1, padx=5)
btn_export = ttk.Button(
    btn_frame, text="Xuất toàn bộ danh sách", command=export_to_excel
)
